...
const statusCode = (res, status, name) => {
  switch (status) {
    ...
    default:
      ...
      /* Set Repo Hook */
      chrome.storage.sync.set({ codeforceshub_hook: res.full_name }, () => {
    ...
    if (xhr.readyState === 4) {
      ...
      if (xhr.status === 200) {
        ...
        if (!bool) {
          ...
          /* Set Repo Hook */
          chrome.storage.sync.set(
            { codeforceshub_hook: res.full_name },
            () => {
              console.log('Successfully set new repo hook');
...
$('#hook_button').on('click', () => {
  ...
  if (!option()) {
    ...
    /* 
        ...
        */
    chrome.storage.sync.get('codeforceshub_token', (data) => {
      const token = data.codeforceshub_token;
      if (token === null || token === undefined) {
        ...
        chrome.storage.sync.get('codeforceshub_username', (data2) => {
          const username = data2.codeforceshub_username;
          if (!username) {
...
chrome.storage.sync.get('mode_type', (data) => {
  ...
  if (mode && mode === 'commit') {
    ...
    chrome.storage.sync.get('codeforceshub_token', (data2) => {
      const token = data2.codeforceshub_token;
      if (token === null || token === undefined) {
        /* Not authorized yet. */
        $('#error').text(
          'Authorization error - Grant codeforceshub access to your GitHub account to continue (click codeforceshub extension on the top right to proceed)',
        );
        $('#error').show();
        $('#success').hide();
        /* Hide accordingly */
        ...
        /* Get access to repo */
        chrome.storage.sync.get('codeforceshub_hook', (repoName) => {
          const hook = repoName.codeforceshub_hook;
          if (!hook) {
            /* Not authorized yet. */
            $('#error').text(
              'Improper Authorization error - Grant codeforceshub access to your GitHub account to continue (clickcodeforceshub extension on the top right to proceed)',
            );
            $('#error').show();
            $('#success').hide();
            /* Hide accordingly */
